...
const injection=`<script>PixelStewardApp.prototype.initFirebase=function(){let cloud={schemaVersion:4,revision:0,generation:'preview',data:${JSON.stringify(fixture)}};if(location.search.includes('empty'))cloud.data=PortfolioCore.empty();this.authUser={email:'preview@local'};this.finnhubApiKey='';this.isFirebaseOnline=true;this.setCloudStatus('online','ข้อมูลจำลอง — ไม่เชื่อมฐานจริง');this.dbRef={transaction:async fn=>{const next=fn(structuredClone(cloud));if(next)cloud=next;return {committed:!!next,snapshot:{val:()=>structuredClone(cloud)}};},once:async()=>({val:()=>structuredClone(cloud)})};this.acceptCloud(cloud);const authButton=document.getElementById('btn-auth-action');if(authButton)authButton.style.display='none';};PixelStewardApp.prototype.fetchWithTimeout=async function(){throw Error('Preview: no network');};PixelStewardApp.prototype.fetchLiveExchangeRate=async function(){return this.exchangeRate;};</script>`;
